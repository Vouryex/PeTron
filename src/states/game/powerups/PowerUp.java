...
public class PowerUp implements Collideable, Paintable {
	...
	public PowerUp(int xPos, int yPos, Color color) {
		...
		this.color = color;
		try {
			shield = ImageIO.read(getClass().getResourceAsStream("/images/powerups/PowerUpShield Spritesheet.png"));
			sideBlade = ImageIO.read(getClass().getResourceAsStream("/images/powerups/PowerUpSideBlade Spritesheet.png"));
			reverse = ImageIO.read(getClass().getResourceAsStream("/images/powerups/PowerUpReverse Spritesheet.png"));
		} catch(IOException e) {
			e.printStackTrace();
		}
